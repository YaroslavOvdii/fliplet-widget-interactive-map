  ...
  methods: {
    mapMarkerData: function mapMarkerData() {
      ...
      var newMarkerData = this.markersData.map(function (marker) {
        var markerData = _.find(_this.widgetData.markers, {
          name: marker.data[_this.markerTypeColumn]
        });
    ...
    toUpdateName: function toUpdateName(index, currentName) {
      ...
      this.$nextTick(function () {
        return _this2.$refs['changename-' + index][0].focus();
      });
    },
    ...
    createNewData: function createNewData() {
      ...
      Fliplet.Modal.prompt({
        ...
        _this3.dataSources.push(ds);

        _this3.selectedDataSource = ds.id;
      });
    },
    chooseExistingData: function chooseExistingData() {
      ...
      Fliplet.Modal.confirm({
        ...
        return _this4.deleteDataSource();
      }).then(function () {
        // Remove from dataSources
        _this4.dataSources = _.filter(_this4.dataSources, function (ds) {
          return ds.id !== _this4.markersDataSourceId;
        });
        _this4.markersDataSource = null;
        _this4.manualSelectDataSource = true;
      });
    },
    ...
    addMarkers: function addMarkers(fromLoad, options) {
      ...
      if (fromLoad) {
        ...
        this.mappedMarkerData.forEach(function (marker, index) {
          if (marker.data.floor === _this5.selectedMarkerData.floor.name) {
            ...
            _this5.pinchzoomer.addMarkers([new Marker(markerElem, {
    ...
    onTapHandler: function onTapHandler(e) {
      ...
      if (!$(e.target).hasClass('marker')) {
        ...
        var markerFound = _.find(markers, function (marker) {
          return marker._vars.name === _this7.selectedMarkerData.marker.data.name;
        });
    ...
    onMarkerHandler: function onMarkerHandler(e) {
      ...
      var name = $(e.target).attr('data-tooltip');
    ...
    getMarkersData: function getMarkersData() {
      ...
      return Fliplet.DataSources.connect(this.markersDataSourceId, {
        offline: false
      }).then(function (connection) {
        // If you want to do specific queries to return your rows
        // See the documentation here: https://developers.fliplet.com/API/fliplet-datasources.html
        _this8.dataSourceConnection = connection; // To keep the connection to update/delete data later on
    ...
    cleanData: function cleanData() {
      ...
      this.mappedMarkerData.forEach(function (marker, index) {
        ...
        newObj.data[_this9.markerNameColumn] = marker.data.name;
        newObj.data[_this9.markerFloorColumn] = marker.data.floor;
        newObj.data[_this9.markerTypeColumn] = marker.data.type;
        newObj.data[_this9.markerXPositionColumn] = marker.data.positionx;
        newObj.data[_this9.markerYPositionColumn] = marker.data.positiony;
        newData.push(newObj);
      });
      return newData;
    },
