  ...
  Utils.initTooltip = function (tooltipElem, tooltipOptions) {
    ...
    if ($.fn.tooltipster != null) {
      ...
      if (Utils.browser.name == "Firefox") {
        ...
        tempOptions.triggerOpen = {
          mouseenter: true
        };
        tempOptions.triggerClose = {
          mouseleave: true
        };
      }
      ...
      if (Utils.browser.name == "Firefox") {
        ...
        tempHtmlOptions.triggerOpen = {
          mouseenter: true
        };
        tempHtmlOptions.triggerClose = {
          mouseleave: true
        };
      }
...
(function (window, $) {
  var FullscreenElem = function FullscreenElem() {};
      ...
      if (enter && isFullscreen && !oldElem.is(curElem)) {
        ...
        if (enter) {
          ...
          if (oldElem != null && oldElem.is(curElem)) {
            ...
            w.scrollTop(fv.bodyScrollTop); //reset elem to null
...
(function (window, $) {
  var ToggleElem = function ToggleElem(elemParam, varsParam) {
    parent.call(this);
    var o = this,
        _elem = null,
        _vars = o._vars,
    ...
    o.vars = function (varsParam, forceUpdate) {
      if (varsParam !== undefined) {
        _super.vars.call(o, varsParam); //if(forceUpdate)
        //{
...
(function (window, $) {
  ...
  var AdaptiveImageLoader = function AdaptiveImageLoader(elemsParam, varsParam) {
    parent.call(this);
    var o = this,
        jO = $(o),
        _queue = [],
    ...
    function update() {
      jO.triggerHandler({
        type: AdaptiveImageLoader.PROGRESS,
        target: o
      });

      if (_queue != null && _queue.length == 0) {
        jO.triggerHandler({
          type: AdaptiveImageLoader.LOAD_COMPLETE,
          target: o
        });
      }
    }
...
(function (window, $) {
  ...
  var Marker = function Marker(elemParam, varsParam) {
...
(function (window, $) {
  ...
  var ElemZoomer = function ElemZoomer(elemHolderParam, elemParam, markersParam, varsParam) {
    parent.call(this);
    var o = this,
        jO = $(o),
        _vars = o._vars,
    ...
    o.elemHolder = function (newElemHolder) {
      if (newElemHolder !== undefined) {
        if (_elemHolder == null || _elemHolder[0] != $(newElemHolder)[0]) {
          ...
          setupHoverZoom();
          jO.triggerHandler({
            type: ElemZoomer.ELEM_HOLDER_CHANGE,
            target: o
          });
        }
      }
    ...
    o.elem = function (newElem, removeOld) {
      if (newElem != null) {
        ...
        if (_elemHolder != null && (_elem == null || !$.contains(_elemHolder[0], $(newElem)[0]))) {
          ...
          _elem = $(newElem).eq(0); //if(!_elemHolder.contains(_elem))
          //{
          ...
          inputHandler.add(new Hammer.Tap({
            event: 'doubletap',
            taps: 2
          }));
          inputHandler.on("hammer.input", onInput);
          inputHandler.on("doubletap", onDoubleTap); //_elem.on("mousewheel", onMouseWheel);
    ...
    o.addMarkers = function (newMarkers) {
      if (_elem != null && newMarkers != undefined) {
        var newMarkersLen = newMarkers.length; //  for(var i = newMarkersLen - 1; i >= 0; i--)

        for (var i = 0; i < newMarkersLen; i++) {
          var newMarker = newMarkers[i];

          if (!_elem.is("img")) {
            _elem.append(newMarker.elem()); //if(_vars.zoomToMarker)
            //{
    ...
    o.transform = function (transformObj, duration) {
      if (transformObj != null) {
        ...
        curZoom = transformObj.zoom || curZoom; //setZoomPosition(curZoom);
    ...
    o.zoomToMarker = function (markerTarget, zoomLevel, duration) {
      if (markerTarget != null) {
        ...
        duration = !isNaN(duration) ? duration : _vars.animDuration; //zoomLevel = 1;
    ...
    o.resetElem = function (forceReset) {
      ...
      if (_elem != null) {
        ...
        if (forceReset || holderWidth != oldHolderWidth || holderHeight != oldHolderHeight || elemWidth != oldElemWidth || elemHeight != oldElemHeight) {
          var oldZoom = curZoom,
              smartZoom = 1;
          oldWidth = curWidth;
          oldHeight = curHeight;
          ...
          if (elemWidth > 0 && elemHeight > 0) {
            ...
            adjustPosition();
            TweenLite.set(_elem, {
              left: elemLeft + tempAdjustLeft,
              top: elemTop + tempAdjustTop,
              right: "auto",
              bottom: "auto",
              x: curX,
              y: curY,
    ...
    function onDoubleTap(e) {
      if (zoomComplete && !mouseMove) {
        var tempZoom = endZoom,
            resetCenter = false; //if(tempZoom >= _vars.doubleTapZoom || tempZoom >= _vars.maxZoom)
    ...
    function onMouseWheel(e) {
      ...
      if (prevGesture == NO_GESTURE && _vars.allowZoom && _vars.allowMouseWheelZoom && !_vars.allowHoverZoom) {
        ...
        oldX = curX;
        oldY = curY; //centerX = e.clientX - elemPosX - curX;
        //centerY = e.clientY - elemPosY - curY;

        centerX = e.originalEvent.clientX - elemPosX - curX;
        centerY = e.originalEvent.clientY - elemPosY - curY; //centerX = (holderWidth * 0.5) - curX - elemLeft;
        ...
        zoomComplete = false;
        handleZoom();
        endZoom = curZoom; //preventScroll = true;
        //}
      }

      var isFF = Utils.browser.name == "Firefox"; //if(preventScroll || !_vars.allowMouseWheelScroll || !zoomComplete)
      ...
      if (preventScroll || !_vars.allowMouseWheelScroll) {
        ...
        if (scrollTarget.length != 0) {
          var sTop = scrollTarget.scrollTop(); //scrollLeft = scrollTarget.scrollLeft();

          if (delta > 0) {
            sTop -= 100; //scrollTarget.scrollTop(sTop - 10);    
          } else {
            sTop += 100; //scrollTarget.scrollTop(sTop + 10);    
          }

          if (window.ScrollToPlugin) {
            TweenLite.to(scrollTarget, _vars.scrollDuration, {
              scrollTo: {
                y: sTop
              }
            });
    ...
    function onInput(e) {
      if (e.pointers.length == 1) {
        if (_vars.allowDrag && !mouseMove && zoomComplete) //if(_vars.allowDrag)
          {
          ...
          if (e.isFinal) {
            prevGesture = NO_GESTURE;
            TweenLite.to(mouseObj, 0.1, {
              onComplete: endTouchInput
            });
            jO.triggerHandler({
              type: ElemZoomer.GESTURE_END,
              target: o
    ...
    function adjustPosition(fromDrag, resetCenter) {
      ...
      if (fromDrag) {
        ...
        overDragY = curY - tempCurY; //make for both scrollOnOverDragX and scrollOnOverDragY
        //also 
    ...
    function handleMarkerZoom(duration) {
      if (_markers.length > 0) {
        ...
        for (var i = 0; i < _markers.length; i++) {
          var marker = _markers[i]; //marker.zoom(curZoom, baseZoom, duration); 
    ...
    function onMouseLeave(e) {
      TweenLite.to(mouseObj, 0.15, {
        onComplete: disableMouseMove
      });
      zoomComplete = false;
  ...
  ElemZoomer.defaultVars = {
    ...
    adjustHeight: 0,
    resizeDuration: -1,
    handleOverDragX: false,
    handleOverDragY: false,
    force3D: false,
    scrollOnOverDragX: false,
    scrollOnOverDragY: false,
    scrollTarget: window,
    fullscreenScaleMode: "proportionalInside",
    fullscreenAdjustWidth: 0,
    fullscreenAdjustHeight: 0,
    overrideFFScroll: false,
    scrollDuration: 0.25,
    tooltipOptions: {},
    adjustLeft: 0,
    adjustTop: 0,
    fullscreenAdjustLeft: 0,
    fullscreenAdjustTop: 0,
    allowDrag: true,
    allowCenterDrag: false,
    zoomToMarker: false,
    initZoom: 1,
    adjustSmartZoom: true,
...
(function (window, $) {
  ...
  var PinchZoomer = function PinchZoomer(elemParam, varsParam, parseControlsParam) {
    ...
    PinchZoomer.triggerHandler({
      type: PinchZoomer.ADD,
      target: o
    });
    ...
    o.fullscreen = function (value) {
      ...
      if ($.type(value) === "boolean") {
        ...
        o.resetElem(true);
        jO.triggerHandler({
          type: PinchZoomer.FULLSCREEN_TOGGLE,
          target: o
        });
      }
    ...
    function onImagesLoaded() {
      ...
      if (elem != null) {
        TweenLite.to(elem, _vars.animDuration, {
          alpha: 1
        });
        jO.triggerHandler({
          type: PinchZoomer.LOAD_COMPLETE,
          target: o
        });
      }
    }
  ...
  PinchZoomer.parseElem = function (elem, elemParent, vars, parseControls) {
    ...
    if (elem.length == 1 && elemParent.length == 1) {
      ...
      elemVars = $.extend({}, {
        id: pzId
      }, PinchZoomer.defaultVars, vars, Utils.stringToObject("{" + elem.data("options") + "}"));
      ...
      if (parseControls && window.ToggleElem !== undefined) {
        ...
        for (i = 0; i < PinchZoomer.BUTTON_CONTROL_NAMES.length; i++) {
          var controlName = PinchZoomer.BUTTON_CONTROL_NAMES[i],
              ...
              defaultVars = {
            onCss: {
              className: controlName + " on"
            },
            offCss: {
              className: controlName + " off"
            }
          },
              controlVars = {};
  ...
  PinchZoomer.triggerHandler = function () {
    $(PinchZoomer.eventObj).triggerHandler.apply(PinchZoomer.jO, arguments); //PZJO.triggerHandler({ type:PinchZoomer.ADD, target:o });
  };
...
/***/ 4:
/*!********************************************!*\
  !*** multi ./vendor/jquery.pinchzoomer.js ***!
  \********************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /Users/hcarneiro/Repos/Fliplet/fliplet-widget-interactive-floorplan/vendor/jquery.pinchzoomer.js */"./vendor/jquery.pinchzoomer.js");


/***/ })

/******/ });
//# sourceMappingURL=jquery.pinchzoomer.js.map